...
def orchestrator(update, freq):
    ...
    for node in node_list:
        ...
        mem_node = nodemem/1024/1024/1024
        proc_node = nodecpu/1000000000
        print(f'CPU Cores: {proc_node:.0f} | Memory:{mem_node:.1f}(GB)')
        # print(f'CPU Cores: {nodecpu/1000000000:.0f} | Memory:{nodemem/1000000000:.1f}(GB)')
...
def service_rabbit_edge():
    ...
    cpu_mem_limits = docker.types.Resources(cpu_limit=1000000000, mem_limit=1073741824)
    if service_list:
        print("Atualizando containers na borda...")
    else:
        print("Criando serviços na Borda...")
        client.services.create('deniscontini/rabbitconsuledge:1.1', name='rabbitmq-edge', mode='global', constraints=['node.labels.layer==edge'], networks=[config.NETWORKS['edge']], env=['RABBITMQ_ERLANG_COOKIE=passwd123'], endpoint_spec=(open_ports_edge), resources=(cpu_mem_limits))
...
def service_rabbit_fog():
    ...
    cpu_mem_limits = docker.types.Resources(cpu_limit=4000000000, mem_limit=4294967296)
    if service_list:
        print("Atualizando containers na névoa...")
    else:
        print("Criando serviços na Névoa...")
        client.services.create('deniscontini/rabbitconsulfog:1.1', name='rabbitmq-fog', mode='global', constraints=['node.labels.layer==fog'], networks=[config.NETWORKS['fog']], env=['RABBITMQ_ERLANG_COOKIE=passwd123'], endpoint_spec=(open_ports_fog), resources=(cpu_mem_limits))
...
def service_rabbit_cloud():
    ...
    if service_list:
        ...
        client.services.create('deniscontini/rabbitconsulcloud:1.1', name='rabbitmq-cloud', mode='global', constraints=['node.labels.layer==cloud'], networks=[config.NETWORKS['cloud']], env=['RABBITMQ_ERLANG_COOKIE=passwd123'], endpoint_spec=(open_ports_cloud))
...
print('6 - Executar orquestrador;\n')
print('7 - Executar orquestrador - Update;\n')
print('8 - List\n')
menu = int(input())
